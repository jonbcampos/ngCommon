...
(function () {
    ...
    var common = angular.module('common.filters.calculateTotalsByDayAndProperty', []);

    /**
     * @ngdoc filter
     * @name common.filters.calculateTotalsByDayAndProperty
     *
     * @description
     * Calculates the total through a dual array.
     *
     ...
     *             <hr/>
     *             {{data | calculateTotalsByDayAndProperty:days:properties }}
     *         </div>
     *     </doc:source>
     * </doc:example>
     *
     */
    common.filter('calculateTotalsByDayAndProperty', function () {
        return function (input, days, properties) {
            ...
            for (var i = 0, n = days.length; i < n; i++) { //days loop
                ...
                for (var j = 0, m = properties.length; j < m; j++) {
                    ...
                    if (input[days[i]][properties[j]] === undefined ||
                        input[days[i]][properties[j]] === null ||
                        isNaN(input[days[i]][properties[j]])) {
                        continue;
                    }
                    total += input[days[i]][properties[j]];
                }
            }
            return total;
        };
